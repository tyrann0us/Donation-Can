            ...
            if (!$wp_rewrite->using_permalinks()) {
                ...
                foreach ($donations as $donation) {
                    ...
                    if (($show_donor_name && !$donation->anonymous) && $show_donation_sum) {
                        if ($goal_id == "__all__") {
                            $donation_string = __("DONATION_LIST_WITH_NAME_AND_SUM_AND_CAUSE", "donation_can");
                        } else {
                            $donation_string = __("DONATION_LIST_WITH_NAME_AND_SUM", "donation_can");
                        }
                    } else if ($show_donor_name && !$donation->anonymous) {
                        if ($goal_id == "__all__") {
                            $donation_string = __("DONATION_LIST_WITH_NAME_AND_CAUSE", "donation_can");
                        } else {
                            $donation_string = __("DONATION_LIST_WITH_NAME", "donation_can");
                        }
                    } else if ($show_donation_sum) {
                        if ($goal_id == "__all__") {
                            $donation_string = __("DONATION_LIST_WITH_SUM_AND_CAUSE", "donation_can");
                        } else {
                            $donation_string = __("DONATION_LIST_WITH_SUM", "donation_can");
                        }
                    } else {
                        if ($goal_id == "__all__") {
                            $donation_string = __("DONATION_LIST_WITH_CAUSE", "donation_can");
                        } else {
                            $donation_string = "";
                        }
                    }
